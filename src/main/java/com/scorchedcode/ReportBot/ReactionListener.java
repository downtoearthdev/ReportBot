...
public class ReactionListener extends ListenerAdapter {
    ...
    public void onMessageReactionAdd(@NotNull MessageReactionAddEvent event) {
        ...
        if(warningAlias.contains("\u26A0")) {
            ...
            ReportManager.getInstance().makeReport(event.getMessageId(), event.getTextChannel().getId(), event.retrieveMessage().complete().getAuthor().getAsTag(), event.getUser().getAsTag());
        }
    }
}
